...
def shortpath_withintervals(net, time,nodes,check,s, t):
    ...
    firsttime=1
    checkagain=s
    thres=(time[0][1]-time[0][0])*3600 # Change it to 60 if adjacent nodes timings are in minutes.
...
##            print i
##            print labels[i]
##            print labels[minNode]
##            print net[minNode][i]
##            print caltime
            if labels[i] > max(labels[minNode], (net[minNode][i]+caltime)):
                ...
                #print caltime
                labels[i] = max (labels[minNode], (net[minNode][i]+caltime))
                newlabel[i]= labels[minNode]+net[minNode][i]+caltime
                
                if (labels[i] <= thres):
                    drop1[i] = labels[minNode] + net[minNode][i]+caltime
                    order[i] = minNode
    ...
    for node_idx in range(len(path) - 1):
        start_node = path[node_idx]
        end_node = path[node_idx+1]
        total_distance += net[start_node][end_node]
        
    return s + "," + t + "," + str(path) + "," + str(total_distance)
check=1
labels={}
net2=copy(net)
for a in nodes:
    for  elem in net[a].keys():
        #print dijkstra(net,time,nodes, check,s=elem, t=a)
        calltime=0
        newneighbor=nearesttime(time,nodes,check,elem,a)
        if time[nodes.index(a)][0]<=newneighbor[1] and newneighbor[0]<=time[nodes.index(a)][1]: #if overlaping
            calltime=0
            #nodetime[i]=0
        else: #adding time with weight if not overlaping
            if newneighbor[0]==time[nodes.index(a)][0]:
                calltime=0
            else:

                if newneighbor[0]<time[nodes.index(a)][0]:
                    calltime=(24+newneighbor[0]-time[nodes.index(a)][1])*3600 # Change it to 60 if adjacent nodes timings are in minutes.
                else:
                    calltime=(abs(newneighbor[0]-time[nodes.index(a)][1])*3600) # Change it to 60 if adjacent nodes timings are in minutes.
        #print  a, elem,  newneighbor, calltime

        net2[a][elem]=net2[a][elem]+calltime

        check=check+1
for a in nodes:
    for  elem in net[a].keys():
        if net2[a][elem]>net2[elem][a]:
            net2[a][elem]=net2[elem][a]
...
for ind in final_ans:
    short=shortpath_withintervals(net2,time, nodes,check,s=str(nodes[int(source)]), t=str(nodes[ind[0]]))
